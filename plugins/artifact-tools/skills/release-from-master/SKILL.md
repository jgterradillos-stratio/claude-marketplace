...
## Step 2 — Check no active pipeline on master/main

Call `get_branch_pipeline_status` with artifact `"$ARGUMENTS"` and branch `"master"`.

- If the tool returns an error indicating the branch or repo was not found (isError / 404), retry with branch `"main"`.
- If the tool returns an error for any other reason: **stop** and report it.
- If any check run has status `queued` or `in_progress`: **stop**. Report: "There is an active pipeline on master. Aborting."
