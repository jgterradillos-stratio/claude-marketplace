...
async function listSubdirectories() {
    ...
    if (!res.ok)
        throw new Error(`Jenkins API error: ${res.status} ${res.statusText}`);
    const data = (await res.json());
    return data.jobs.map((j) => j.name);
}
async function findProjectInSubdir(subdir, project) {
...
async function getVersionFromRef(artifact, ref) {
    const encodedRef = encodeURIComponent(ref);
    // VERSION file
    const versionRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/VERSION?ref=${encodedRef}`, { headers: githubHeaders() });
    if (versionRes.ok) {
        const data = (await versionRes.json());
        return Buffer.from(data.content, "base64").toString("utf-8").trim();
    }
    // package.json (Node/TypeScript projects)
    const pkgRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/package.json?ref=${encodedRef}`, { headers: githubHeaders() });
    if (pkgRes.ok) {
    ...
    const pomRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/pom.xml?ref=${encodedRef}`, { headers: githubHeaders() });
    if (pomRes.ok) {
...
async function getMasterVersion(artifact) {
    return getVersionFromRef(artifact, "master");
}
async function triggerBuild(subdir, project, nextVersion) {
...
export function registerJenkinsTools(server) {
    server.registerTool("list_jenkins_subdirectories", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error connecting to Jenkins: ${e.message}` }],
                isError: true,
            };
        }
    ...
    server.registerTool("trigger_jenkins_build", {
...
AFTER triggering: if a queue_url is returned, use ScheduleWakeup with delaySeconds=60 to poll get_jenkins_build_status. In each poll pass both queue_url and build_url (once known from a previous poll response). Continue until status is success/failure/aborted, then notify the user in chat with the final result.`,
        inputSchema: {
            ...
            version: z
                .string()
                .optional()
                .describe("Version to build. For prerelease/milestone/release use x.y.z. For branch use x.y (e.g. '1.2'). For milestone from master, omit to auto-read from VERSION/package.json."),
            subdirectory: z
                .string()
                .optional()
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error querying Jenkins: ${e.message}` }],
                isError: true,
            };
        }
        if (!projectName) {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error triggering build: ${e.message}` }],
                isError: true,
            };
        }
        if (!result.ok) {
    ...
    server.registerTool("get_jenkins_build_status", {
...
- On completion notify the user: always show buildUrl, status, and if failure show consoleUrl + errorCause as a bullet list.`,
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Network error querying Jenkins: ${e.message}` }],
                isError: true,
            };
        }
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Network error querying build: ${e.message}` }],
                isError: true,
            };
        }
        let status;
