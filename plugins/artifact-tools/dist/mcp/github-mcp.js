...
function formatRelease(library, release) {
    const publishDate = new Date(release.published_at).toLocaleDateString("es-ES", { year: "numeric", month: "long", day: "numeric" });
...
async function fetchReadmeSummary(artifact) {
    const res = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/readme`, { headers: githubHeaders() });
    if (!res.ok)
        return null;
...
async function fetchSnapshotVersion(artifact, ref) {
    const refsToTry = ref ? [ref] : ["master", "main"];
    for (const r of refsToTry) {
        // VERSION file
        const versionRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/VERSION?ref=${r}`, { headers: githubHeaders() });
        if (versionRes.ok) {
            const data = (await versionRes.json());
            return Buffer.from(data.content, "base64").toString("utf-8").trim();
        }
        // package.json (Node/TypeScript projects)
        const pkgRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/package.json?ref=${r}`, { headers: githubHeaders() });
        if (pkgRes.ok) {
        ...
        const pomRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/contents/pom.xml?ref=${r}`, { headers: githubHeaders() });
        if (pomRes.ok) {
            const data = (await pomRes.json());
...
function compareTagsDesc(a, b) {
    const nums = (tag) => baseVersion(tag).split(/[.\-]/).map(Number);
...
async function branchExists(artifact, branch) {
    const url = `${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/branches/${branch}`;
    const res = await fetch(url, { headers: githubHeaders() });
    return res.ok;
}
async function listBranches(artifact) {
    const branches = [];
    let page = 1;
    while (true) {
        const res = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/branches?per_page=100&page=${page}`, { headers: githubHeaders() });
        if (!res.ok)
            break;
        const data = (await res.json());
        if (data.length === 0)
...
export function registerGithubTools(server) {
    server.registerTool("get_latest_release", {
        description: "Get the latest release of a Stratio artifact from GitHub. Use the 'type' parameter to query stable releases, snapshots (version under development on master), milestones (x.y.z-M<number>), or prereleases (x.y.z-rc<alphanumeric> or x.y.z-<alphanumeric>). Always reproduce the complete response verbatim in your reply — do not summarize or omit any field.",
        inputSchema: {
            artifact: z
                .string()
            ...
            type: z
                ...
                .optional()
                .describe("Type of version to query: 'release' (default, latest stable), 'snapshot' (current master version), 'milestone' (x.y.z-M<number>), 'prerelease' (x.y.z-rc<alphanumeric> or x.y.z-<alphanumeric>)"),
        },
        ...
        if (type === "milestone" || type === "prerelease") {
            ...
            const match = releases.find((r) => type === "milestone" ? isMilestoneTag(r.tag_name) : isPrereleaseTag(r.tag_name));
            if (!match) {
                return {
    ...
    server.registerTool("get_artifact_versions", {
        description: "Get all available versions of a Stratio artifact: master/main snapshot (in active development), latest stable releases, milestones, prereleases, and whether the maintenance release branch (branch-x.y) exists on GitHub. Always reproduce the complete response verbatim in your reply — do not summarize or omit any field.",
        inputSchema: {
        ...
        const top5 = (tags) => [...tags].sort(compareTagsDesc).slice(0, 5).join(", ") || "none";
    ...
    server.registerTool("get_artifact_info", {
    ...
    server.registerTool("search_artifacts", {
        ...
        const res = await fetch(`${GITHUB_API}/search/repositories?q=${encodeURIComponent(query)}+in:name+org:${STRATIO_ORG}&per_page=50&sort=name`, { headers: githubHeaders() });
        if (!res.ok) {
            return {
        ...
        const rows = data.items.map((r) => `| ${r.name} | ${r.language === "Mustache" ? "Helm" : (r.language ?? "—")} | ${r.html_url} |`);
        return { content: [{ type: "text", text: [header, ...rows].join("\n") }] };
    });
    server.registerTool("list_artifact_branches", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error fetching branches for "${artifact}": ${e.message}` }],
                isError: true,
            };
        }
    ...
    server.registerTool("get_branch_pipeline_status", {
        description: "Check whether a CI pipeline is currently running for a specific branch of a Stratio artifact. Queries GitHub Check Runs for the branch HEAD commit and reports the status (queued, in_progress, or completed with conclusion) of each check run. If any check run is queued or in_progress, use ScheduleWakeup with delaySeconds=60 to poll again — do NOT call this tool in a tight loop. Always reproduce the complete response verbatim in your reply — do not summarize or omit any field.",
        inputSchema: {
            artifact: z.string().describe("Name of the Stratio artifact/repository on GitHub"),
            branch: z
                .string()
    ...
    server.registerTool("get_pr_pipeline_status", {
        description: "Check the CI pipeline status for a Stratio GitHub pull request. Queries GitHub Check Runs for the PR's head commit SHA and reports the status (queued, in_progress, or completed with conclusion) of each check run. Use this instead of get_branch_pipeline_status when you have a PR number. If any check run is queued or in_progress, use ScheduleWakeup with delaySeconds=60 to poll again — do NOT call this tool in a tight loop. Always reproduce the complete response verbatim in your reply — do not summarize or omit any field.",
        inputSchema: {
            artifact: z.string().describe("Name of the Stratio artifact/repository on GitHub"),
            pull_number: z.number().describe("Pull request number"),
        },
    }, async ({ artifact, pull_number }) => {
        const prRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/pulls/${pull_number}`, { headers: githubHeaders() });
        if (prRes.status === 404) {
            return {
    ...
    server.registerTool("get_pull_request_info", {
        description: "Get information about a specific pull request in a Stratio GitHub repository: title, state, head branch, base branch, head SHA, author, draft status, and mergeable flag. Always reproduce the complete response verbatim in your reply.",
        inputSchema: {
            artifact: z.string().describe("Name of the Stratio artifact/repository on GitHub"),
            pull_number: z.number().describe("Pull request number"),
        },
    }, async ({ artifact, pull_number }) => {
        const res = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/pulls/${pull_number}`, { headers: githubHeaders() });
        if (res.status === 404) {
            return {
                content: [{ type: "text", text: `PR #${pull_number} not found in "${artifact}".` }],
                isError: true,
    ...
    server.registerTool("merge_pull_request", {
        description: "Merge a Stratio GitHub pull request using squash merge, bypassing branch protection rules (requires admin token). Combines all commits into a single squash commit. Always reproduce the complete response verbatim in your reply.",
        inputSchema: {
            artifact: z.string().describe("Name of the Stratio artifact/repository on GitHub"),
        ...
        const mergeRes = await fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${artifact}/pulls/${pull_number}/merge`, {
            method: "PUT",
            headers: {
                ...githubHeaders(),
        ...
        if (!mergeRes.ok) {
            ...
            if (mergeRes.status === 403)
                hint = " (insufficient permissions — admin token required to bypass branch protection)";
            if (mergeRes.status === 405)
                hint = " (squash merge may be disabled on this repo, or the PR is already merged)";
            if (mergeRes.status === 409)
                hint = " (merge conflict or SHA mismatch — resolve conflicts before merging)";
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("get_repo_activity", {
        ...
        const repoNames = repositories.map((r) => r.startsWith("http") ? r.split("/").pop() : r);
        const results = [];
        for (const repo of repoNames) {
            const [commitsRes, prsRes] = await Promise.all([
                fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${repo}/commits?since=${sinceISO}&per_page=50`, { headers: githubHeaders() }),
                fetch(`${GITHUB_API}/repos/${STRATIO_ORG}/${repo}/pulls?state=open&per_page=50&sort=updated`, { headers: githubHeaders() }),
            ]);
            const activity = { repo, commits: [], open_prs: [] };
